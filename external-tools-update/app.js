...
function createCourseObject (course, externalTool) {
  const shortObject = {
    courseId: course.id,
    ...
    rootAccountId: course.root_account_id,
    toolId: externalTool.id,
    toolName: externalTool.name,
    toolDomain: externalTool.domain,
    toolUrl: externalTool.url,
    toolConsumerKey: externalTool.consumer_key
  }
  return shortObject
}
...
async function getAllTools (course) {
  ...
  for await (const externalTool of canvas.list(toolSearchUrl)) {
    foundTools.push(createCourseObject(course, externalTool))
  };
}
...
async function updateTool (courseId, toolId) {
  ...
  console.log(createCourseObject(course.body, updatedTool.body))
}
