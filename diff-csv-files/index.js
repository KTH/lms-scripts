...
async function diff () {
  ...
  for (const line of provisioningFilecontent) {
    const [ , sis_id,,, login_id,,,,, email,, created_by_sis ] = line.split(',') // eslint-disable-line
  ...
  for (const line of ugFilecontent) {
    const [ sis_id, username, email ] = line.split(',') // eslint-disable-line
    ugUsers[sis_id] = {
      sis_id, username, email
    }
  }
